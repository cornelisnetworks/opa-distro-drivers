...
#define HFI1_DMS_WORK_ITEM_MAX_RETRY_TIME_NS  (5000000000) //   5 s
#define HFI1_DMS_ACCESS_MAX_RETRY_TIME_NS      (100000000) // 100 ms
#define HFI1_DMS_STALE_POLL_TIME_NS             (10000000) //  10 ms
#define HFI1_DMS_STALE_THRESHOLD_TIME_NS      (1000000000) //   1 s
#define HFI1_DMS_FABRIC_PACKET_MAX_LIFETIME_NS (500000000) // 500 ms
...
static void hfi1_dms_tracker_timestamp_update(struct hfi1_dms *dms, union hfi1_dms_tracker *tracker)
{
	DMS_BUG_ON(!tracker);
...
static u16 _rift_key_index(u16 const key)
{
	return key & HFI1_DMS_RIFT_IDX_MASK;
}

static u16 _rift_key_generation(u16 const key)
{
	return (key & HFI1_DMS_RIFT_GEN_MASK) >> HFI1_DMS_RIFT_IDX_BITS;
}

static enum hfi1_dms_rift_err _rift_key_error(u16 const key)
{
...
static void _rift_assign(struct hfi1_dms_rift *rift, union hfi1_dms_tracker *tracker, u16 key)
{
	static u16 const sz = HFI1_DMS_ARRAY_SIZE(rift->arr);
...
static union hfi1_dms_tracker * _rift_lookup(struct hfi1_dms_rift *rift, u16 key)
{
	static u16 const sz = HFI1_DMS_ARRAY_SIZE(rift->arr);
	u16 const idx = _rift_key_index(key);
...
static void _rift_release(struct hfi1_dms_rift *rift, u16 key)
{
	DMS_BUG_ON(!rift);
...
static void _rift_cancel(struct hfi1_dms_rift *rift, u16 key) {
...
static int _dms_tx_rift_poll(struct hfi1_dms *dms)
{
	int ret;
...
static u32 hfi1_dms_tracker_stale(union hfi1_dms_tracker *tracker, ktime_t const now, u64 const max_elapsed_ns)
{
...
static void hfi1_dms_tx_tracker_cancel(struct hfi1_dms *dms, struct hfi1_dms_tx_tracker *tx_tracker, int err)
{
	DMS_BUG_ON(!dms);
	DMS_BUG_ON(!tx_tracker);
...
static void hfi1_dms_rx_tracker_cancel(struct hfi1_dms *dms, struct hfi1_dms_rx_tracker *rx_tracker, int err)
{
	DMS_BUG_ON(!dms);
	DMS_BUG_ON(!rx_tracker);
