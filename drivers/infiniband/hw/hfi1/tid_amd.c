...
static int amd_page_iter_next(struct hfi1_page_iter *diter)
{
	 ...
	 * is TID-compatible.
	 */
	if (iter->sg && !IS_ALIGNED(__amd_page_iter_dma_addr(iter), EXP_TID_ADDR_SIZE))
		ret = -EFAULT;
	else
			     ...
			     struct tid_user_buf **tbuf)
{
	...
	if (!IS_ALIGNED(vaddr, EXP_TID_ADDR_SIZE))
		return -EINVAL;
...
static void amd_user_buf_kref_cb(struct kref *ref)
{
	...
	spin_lock(&abuf->pages_lock);
	trace_unpin_recv_mem(abuf->fd, HFI1_MEMINFO_TYPE_AMD, 0, abuf, abuf->common.vaddr,
			     abuf->common.length,
...
static void amd_free_cb(void *priv)
{
	...
	spin_lock(&abuf->pages_lock);
	trace_invalidate_recv_mem(abuf->fd, HFI1_MEMINFO_TYPE_AMD, abuf->common.vaddr,
			 ...
			 struct tid_user_buf *tbuf)
{
	...
	trace_pin_recv_mem(fd, HFI1_MEMINFO_TYPE_AMD, tbuf->vaddr, tbuf->length, 0, 0,
...
fail:
	abuf->pages = NULL;
	trace_pin_recv_mem(fd, HFI1_MEMINFO_TYPE_AMD, tbuf->vaddr, tbuf->length, ret, 0,
			   abuf, 0, 0);
