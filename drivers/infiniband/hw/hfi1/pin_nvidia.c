 ...
 */
static void nvidia_pintree_remove(struct nvidia_pintree_node *n, struct list_head *l,
				  bool external_eviction)
{
	struct nvidia_pintree *cache = n->pintree;

	interval_tree_remove(&n->node, &cache->root);
	if (l)
		list_move(&n->lru_node, l);
	else
		list_del(&n->lru_node);
	cache->size -= n->size;
	if (external_eviction)
		...
		cache->internal_evictions++;
}

static void unpin_nvidia_node(struct nvidia_pintree_node *n)
{
	struct nvidia_pq_state *state = n->pintree->state;
	struct hfi1_user_sdma_pkt_q *pq = state->pq;
	int ret = 0;

	/* There must be no I/O referencing this node. */
	WARN_ON(atomic_read(&n->refcount) != 0);

	PIN_PQ_DBG(pq, "unmapping node pages");
	if (n->mapping)
		ret = rdma_interface.dma_unmap_pages(state->hfi_dev, n->page_table, n->mapping);
	WARN_ON(ret);
	n->mapping = NULL;

	PIN_PQ_DBG(pq, "putting node pages");
	if (n->page_table)
		ret = rdma_interface.put_pages(/* uint64_t p2p_token */ 0, /* va_space_token */ 0,
					       n->node.start, n->page_table);
	WARN_ON(ret);
	n->page_table = NULL;
}

static void free_nvidia_node(struct nvidia_pintree_node *n)
{
	unpin_nvidia_node(n);
	kfree(n);
}
...
/*
 * @n->pintree==NULL when called.
 */
static void free_nvidia_node_from_cb(struct nvidia_pintree_node *n)
{
	int result;
	 ...
	 */
	while (atomic_read(&n->refcount) > 0)
		rep_nop();

	result = rdma_interface.free_dma_mapping(n->mapping);
	WARN_ON(unlikely(result));
	n->mapping = NULL;

	result = rdma_interface.free_page_table(n->page_table);
	WARN_ON(unlikely(result));
	n->page_table = NULL;
...
static void free_pintree(struct nvidia_pintree *pintree)
{
	...
	while (1) {
		...
		nvidia_pintree_remove(found, NULL, false);
		spin_unlock(&pintree->lock);
 ...
 *
 * Locking in nvidia guarantees that if free_pintree() is running concurrent to
 * this, that CPU will not get past nvidia_p2p_put_pages() before this returns.
 * I.e. node from @data will not be kfree()'d until after this returns.
 *
...
static void remove_nvidia_pages(void *data)
{
	...
	trace_invalidate_sdma_mem(pintree, HFI1_MEMINFO_TYPE_NVIDIA, node->node.start,
				  node->size, node);
	found_node = interval_tree_iter_first(&pintree->root, node->node.start, node->node.last);
	found = container_of(found_node, struct nvidia_pintree_node, node);
	if (found == node) {
		nvidia_pintree_remove(node, NULL, true);
		spin_unlock(&pintree->lock);
		/*
		 * pintree can be destroyed as soon as node is removed from it;
		 * not safe to dereference node->pintree after this.
		 */
		node->pintree = NULL;
		free_nvidia_node_from_cb(node);
	} else {
		/*
		 * node either not yet inserted into cache or already removed
		 * from cache. Either way, another CPU is responsible for
		 * freeing node.
		 */
		spin_unlock(&pintree->lock);
				 ...
				 struct nvidia_pintree_node *node)
{
	...
	if (existing) {
		result = -EEXIST;
	} else if (node->invalidated) {
		/* NVIDIA mem freed before node could be added to cache */
		result = -EFAULT;
	} else {
		/* This will become the packet's reference */
...
static bool evict_nvidia_pinnings(struct nvidia_pintree *pintree, size_t goal, bool internal)
{
	...
	list_for_each_entry_safe(cur, tmp, &pintree->lru_list, lru_node) {
		if (atomic_read(&cur->refcount) == 0) {
			nvidia_pintree_remove(cur, &evict_list, !internal);
			total += cur->size;
			if (total >= goal)
				break;
		}
	...
	list_for_each_entry_safe(cur, tmp, &evict_list, lru_node)
		free_nvidia_node(cur);
	trace_evict_sdma_mem(pintree, HFI1_MEMINFO_TYPE_NVIDIA, total, goal);
			   ...
			   u64 start, u64 len)
{
	...
	while (1) {
		...
		if (!node) {
			...
			trace_pin_sdma_mem(pintree, HFI1_MEMINFO_TYPE_NVIDIA,
					   start, (last + 1 - start),
					   0, 0, *node_p,
		...
		if (node->node.start <= start) {
			*node_p = node;
			...
			trace_pin_sdma_mem(pintree, HFI1_MEMINFO_TYPE_NVIDIA, start,
...
static int add_from_entry(struct hfi1_user_sdma_pkt_q *pq,
			  struct user_sdma_txreq *tx,
			  struct nvidia_pintree_node *e,
			  size_t start, size_t from_this)
{
	...
	while (from_this) {
		page_index = (start - e->node.start) >>
				NVIDIA_PAGE_SHIFT_X(e->mapping->page_size_type);
		page_offset =
			start & NVIDIA_PAGE_OFFSET_MASK_X(e->mapping->page_size_type);
		from_page =
			NVIDIA_PAGE_SIZE_X(e->mapping->page_size_type) - page_offset;
		if (from_page > from_this)
			from_page = from_this;

		dma_start = e->mapping->dma_addresses[page_index] + page_offset;
		ret = sdma_txadd_daddr(pq->dd, &tx->txreq, dma_start, from_page);
		if (ret) {
			/*
...
static int add_from_iovec(struct nvidia_pq_state *state,
			  struct user_sdma_request *req,
			  struct user_sdma_txreq *tx,
			  struct user_sdma_iovec *iov,
			  size_t from_iovec)
{
	struct nvidia_pintree *cache;
	u32 device_id;

	device_id = iov->context;
	cache = get_nvidia_pintree(state, device_id);
	if (!cache)
		return -ENOMEM;
	...
	while (from_iovec > 0) {
		...
		start = (uintptr_t)iov->iov.iov_base + iov->offset;
		ret = get_cache_entry(cache, &e, start, from_iovec);
		if (ret) {
			SDMA_DBG(req, "SDMA pin nvidia segment failed %d", ret);
			return ret;
		}

		/* transfer cleanup responsibility to tx */
		...
		from_this = (e->node.last + 1) - start;
		if (from_this > from_iovec)
			from_this = from_iovec;
		ret = add_from_entry(req->pq, tx, e, start, from_this);
		if (ret) {
			SDMA_DBG(req, "SDMA txreq add nvidia segment failed %d", ret);
			return ret;
		}
		iov->offset += from_this;
		from_iovec -= from_this;
	}
...
{
	struct nvidia_pq_state *state =
		PINNING_STATE(req->pq, HFI1_MEMINFO_TYPE_NVIDIA);
	size_t rem;
	 ...
	 */
	rem = *remaining;
	while ((rem > 0) && (iov->type == HFI1_MEMINFO_TYPE_NVIDIA)) {
		struct user_sdma_iovec *cur;
		size_t from_this;
		int ret;

		cur = iov;
		from_this = iov->iov.iov_len - iov->offset;
		if (from_this > rem) {
			from_this = rem;
		} else {
		...
		ret = add_from_iovec(state, req, tx, cur, from_this);
		if (ret)
			return ret;
			    ...
			    struct hfi1_pin_stats *stats)
{
	...
	stats->memtype = HFI1_MEMINFO_TYPE_NVIDIA;
	spin_lock(&state->lock);
...
/**
 * @return 0 on success, EOPNOTSUPP on symbol_get() failure.
 */
static int hfi1_sdma_nvidia_init(void)
{
	const char *err_str;
	char name_buf[64];
	int ret;
...
#define GET_SYMBOL(name)					\
	...
	if (!rdma_interface.name) {				\
		...
		ret = EOPNOTSUPP; /* Note: not -EOPNOTSUPP */	\
		goto fail;					\
	}
...
int hfi1_pin_nvidia_init(void)
{
	int ret;

	ret = hfi1_sdma_nvidia_init();
	if (ret)
		goto bail;

	ret = hfi1_tid_nvidia_init();
	if (ret)
		goto bail_sdma;
...
bail_tid:
	hfi1_tid_nvidia_free();
bail_sdma:
	hfi1_sdma_nvidia_free();
bail:

	/*
	 * EOPNOTSUPP (not -EOPNOTSUPP) means symbol_get() failed; don't return
	 * error so hfi1 will continue without nvidia support.
	 */
	if (ret == EOPNOTSUPP) {
		pr_info("hfi1 Nvidia SDMA/TID/GDRCopy support disabled\n");
		ret = 0;
	}
...
void hfi1_pin_nvidia_free(void)
{
