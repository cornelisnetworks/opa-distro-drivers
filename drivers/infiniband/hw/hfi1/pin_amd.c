...
/* Registration/deregistration declarations */
static void hfi1_sdma_amd_free(void);

/* AMD-TID declarations */
int hfi1_tid_amd_init(void);
void hfi1_tid_amd_free(void);
...
static void unpin_amd_node(struct amd_pintree_node *node)
{
	...
	node->p2p_info = NULL;
	trace_unpin_sdma_mem(pintree, HFI1_MEMINFO_TYPE_AMD, ret, node, va, va_len,
			     dma, dma_len);
				  ...
				  unsigned long cur_seq)
{
	...
	trace_invalidate_sdma_mem(cache, HFI1_MEMINFO_TYPE_AMD, n->node.start,
				  n->size, n);
...
/*
 * On success, kref_get() will have been called on @node->ref. This kref
...
static bool evict_amd_pinnings(struct amd_pintree *pintree, size_t goal, bool internal)
{
	...
	trace_evict_sdma_mem(pintree, HFI1_MEMINFO_TYPE_AMD, total, goal);
...
/*
 * If matching node is found, kref_get() will be called on node->ref.
 */
			   ...
			   u64 last, size_t tree_size)
{
	...
	/*
	 * When amd_use_cache is true, this kref becomes the cache's ref.
	 * When amd_use_cache is false, this kref is the caller's ref.
	 */
	kref_init(&node->ref);
				      ...
				      u64 start, u64 last)
{
	...
	while (1) {
		...
		if (!node) {
			...
			trace_pin_sdma_mem(pintree, HFI1_MEMINFO_TYPE_AMD, start,
					...
					size_t from_this_iovec)
{
	...
	while (from_this_iovec > 0) {
		...
		ret = hfi1_user_sdma_add_ref(tx, cache_entry, HFI1_MEMINFO_TYPE_AMD);
		/* tx owns cache_entry->ref now */
		if (ret) {
			amd_node_put(cache_entry);
			return ret;
		}
			 ...
			 struct hfi1_pin_stats *stats)
{
	...
	stats->memtype = HFI1_MEMINFO_TYPE_AMD;
	spin_lock(&state->lock);
...
/**
 * @return 0 on success, EOPNOTSUPP on symbol_get() failure, not {0,EOPNOTSUPP}
 * for all other errors.
 */
static int hfi1_sdma_amd_init(void)
{
	const char *err_str;
	char name_buf[64];
	int ret;
...
int hfi1_pin_amd_init(void)
{
	int ret;

	ret = hfi1_sdma_amd_init();
	if (ret)
		goto bail;
	ret = hfi1_tid_amd_init();
	if (ret)
		goto bail;
...
bail:
	 ...
	 * error so hfi1 will continue without amdgpu support.
	 */
	if (ret == EOPNOTSUPP) {
		pr_info("hfi1 AMD SDMA/TID support disabled\n");
		ret = 0;
	}
